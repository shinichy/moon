...
///
/// Used by [MyRenderEditable.onSelectionChanged].
@Deprecated(
  'Signature of a deprecated class method, '
      'textSelectionDelegate.userUpdateTextEditingValue. '
      'This feature was deprecated after v1.26.0-17.2.pre.',
)
typedef SelectionChangedHandler = void Function(TextSelection selection, MyRenderEditable renderObject, SelectionChangedCause cause);
...
/// [MyVerticalCaretMovementRun.current]) to the previous line, and in case
...
/// the [MyVerticalCaretMovementRun] must not be used. The [isValid] property must
/// be checked before calling [movePrevious] and [moveNext], or accessing
/// [current].
class MyVerticalCaretMovementRun extends BidirectionalIterator<TextPosition> {
  MyVerticalCaretMovementRun._(
      this._editable,
      this._lineMetrics,
  ...
  final MyRenderEditable _editable;

  bool _isValid = true;
  /// Whether this [MyVerticalCaretMovementRun] can still continue.
  ///
  /// A [MyVerticalCaretMovementRun] run is valid if the underlying text layout
  /// hasn't changed.
  ///
...
class MyRenderEditable extends RenderBox with RelayoutWhenSystemFontsChangeMixin, ContainerRenderObjectMixin<RenderBox, TextParentData>, RenderBoxContainerDefaultsMixin<RenderBox, TextParentData> implements TextLayoutMetrics {
  ...
  /// [MyRenderEditable]'s text content.
  ///
  ...
  ///
  /// [MyRenderEditable] is responsible for calculating the [Offset] of this
  ...
  ///
  /// Since [MyRenderEditable] does not handle selection manipulation
  ...
  void _layoutText({ double minWidth = 0.0, double maxWidth = double.infinity }) {
    ...
    const double textMaxWidth = double.infinity;
  ...
  /// Starts a [MyVerticalCaretMovementRun] at the given location in the text, for
  ...
  ///
  /// The [MyVerticalCaretMovementRun.isValid] property indicates whether the text
  /// layout has changed and the vertical caret run is invalidated.
  ///
  /// The caller should typically discard a [MyVerticalCaretMovementRun] when
  /// its [MyVerticalCaretMovementRun.isValid] becomes false, or on other
  /// occasions where the vertical caret run should be interrupted.
  MyVerticalCaretMovementRun startVerticalCaretMovement(TextPosition startPosition) {
    ...
    return MyVerticalCaretMovementRun._(
      this,
      metrics,
      startPosition,
      currentLine.key,
...
class _RenderEditableCustomPaint extends RenderBox {
  ...
  @override
  MyRenderEditable? get parent => super.parent as MyRenderEditable?;
  ...
  @override
  void paint(PaintingContext context, Offset offset) {
    final MyRenderEditable? parent = this.parent;
    assert(parent != null);
...
/// An interface that paints within a [MyRenderEditable]'s bounds, above or
...
/// blocks). It can paint independently from its [MyRenderEditable], allowing it
/// to repaint without triggering a repaint on the entire [MyRenderEditable] stack
/// when only auxiliary content changes (e.g. a blinking cursor) are present. It
/// will be scheduled to repaint when:
///
///  * It's assigned to a new [MyRenderEditable] and the [shouldRepaint] method
///    returns true.
///  * Any of the [MyRenderEditable]s it is attached to repaints.
///  * The [notifyListeners] method is called, which typically happens when the
///    painter's attributes change.
///
/// See also:
///
///  * [MyRenderEditable.foregroundPainter], which takes a [RenderEditablePainter]
///    and sets it as the foreground painter of the [RenderEditable].
///  * [MyRenderEditable.painter], which takes a [RenderEditablePainter]
...
abstract class RenderEditablePainter extends ChangeNotifier {
  ...
  /// is provided to a [MyRenderEditable].
  ///
  ...
  /// [MyRenderEditable]s it attaches to repaint, even if [shouldRepaint] returns
  /// false.
  bool shouldRepaint(RenderEditablePainter? oldDelegate);

  /// Paints within the bounds of a [MyRenderEditable].
  ///
  /// The given [Canvas] has the same coordinate space as the [MyRenderEditable],
  /// which may be different from the coordinate space the [MyRenderEditable]'s
  /// [TextPainter] uses, when the text moves inside the [MyRenderEditable].
  ///
  /// Paint operations performed outside of the region defined by the [canvas]'s
  /// origin and the [size] parameter may get clipped, when [MyRenderEditable]'s
  /// [MyRenderEditable.clipBehavior] is not [Clip.none].
  void paint(Canvas canvas, Size size, MyRenderEditable renderEditable);
}
...
class _FloatingCursorPainter extends RenderEditablePainter {
  ...
  void paintRegularCursor(Canvas canvas, MyRenderEditable renderEditable, Color caretColor, TextPosition textPosition) {
