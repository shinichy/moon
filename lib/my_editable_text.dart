...
class MyEditableText extends StatefulWidget {
  ...
  ///
  /// The cursor refers to the blinking caret when the [MyEditableText] is focused.
  /// {@endtemplate}
  ///
  /// See also:
  ///
  ...
  /// [CupertinoTextField], and [MyEditableText].
  ///
  ...
  ///
  /// The [MyEditableText] widget used on its own will not trigger the display
  /// of the selection toolbar by itself. The toolbar is shown by calling
  /// [MyEditableTextState.showToolbar] in response to an appropriate user event.
  ///
  /// See also:
  ///
  ///  * [CupertinoTextField], which wraps an [MyEditableText] and which shows the
  ///    selection toolbar upon user events that are appropriate on the iOS
  ///    platform.
  ///  * [TextField], a Material Design themed wrapper of [MyEditableText], which
  ...
  ///
  /// The [mouseCursor] is the only property of [MyEditableText] that controls the
  ...
  ///
  /// When null, this widget's [MyEditableTextState] will be used as the
  ...
  /// [MyEditableText].
  ///
  /// If a restoration id is provided, the [MyEditableText] will persist its
  /// current scroll offset and restore it during state restoration.
  ///
  /// The scroll offset is persisted in a [RestorationBucket] claimed from
  /// the surrounding [RestorationScope] using the provided restoration ID.
  ///
  /// Persisting and restoring the content of the [MyEditableText] is the
...
class MyEditableTextState extends State<MyEditableText> with AutomaticKeepAliveClientMixin<MyEditableText>, WidgetsBindingObserver, TickerProviderStateMixin<MyEditableText>, TextSelectionDelegate implements TextInputClient, AutofillClient {
  Timer? _cursorTimer;
  ...
  /// update via [updateEditingValue], or when [MyEditableText] calls
  ...
  ///
  /// Throws an error in debug mode if this [MyEditableText] is not in a batch
  /// edit.
  void endBatchEdit() {
    _batchEditDepth -= 1;
    assert(
  ...
  void _cursorWaitForStart(Timer timer) {
    ...
    assert(!MyEditableText.debugDeterministicCursor);
    _cursorTimer?.cancel();
  ...
  void _startCursorTimer() {
    ...
    if (MyEditableText.debugDeterministicCursor)
      return;
    if (widget.cursorOpacityAnimates) {
  ...
  MyRenderEditable get renderEditable => _editableKey.currentContext!.findRenderObject()! as MyRenderEditable;
...
class _DeleteTextAction<T extends DirectionalTextEditingIntent> extends ContextAction<T> {
  ...
  final MyEditableTextState state;
...
class _UpdateTextSelectionToAdjacentLineAction<T extends DirectionalCaretMovementIntent> extends ContextAction<T> {
  ...
  MyVerticalCaretMovementRun? _verticalMovementRun;
  ...
  void invoke(T intent, [BuildContext? context]) {
    ...
    final MyVerticalCaretMovementRun currentRun = _verticalMovementRun
