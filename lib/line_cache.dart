...
class LineCache {
  ...
  Tuple2<String, List<int>> fromVisualLine(
      RopeNode text, VisualLine line, Selection selection, int lastPos) {
    final startPos = line.interval.start;
    final pos = line.interval.end;
    var cursors = <int>[];
    for (var region in selection.regionsInRange(startPos, pos)) {
      // cursor
      final c = region.end;
      if ((c > startPos && c < pos) ||
          (!region.isUpstream() && c == startPos) ||
          (region.isUpstream() && c == pos) ||
          (c == pos && c == lastPos)) {
        cursors.add(c - startPos);
      }
    }

    return Tuple2(text.sliceToCow(Range(startPos, pos)), cursors);
  }

  void fromVisualLines(
      RopeNode rope, VisualLines visualLines, Selection selection) {
    var lines = visualLines.where((l) => l != null).map((line) {
      final tuple = fromVisualLine(rope, line!, selection, rope.len());
      final text = tuple.item1;
      final cursor = tuple.item2;

      return Line(TextSpan(text: text, style: style), cursor, <int>[]);
    });
    _lines = lines.toList();
  }
...
List<int>? _transformCursor(String s, Map<String, dynamic> json) {
  ...
  return cursor.map((int offset) => _utf8ToUtf16Offset(s, offset)).toList();
}
